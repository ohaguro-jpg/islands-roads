...
function playGame(size, diff, seed) {
  ...
  while (run('!state.gameOver') && round < 300) {
    ...
    // 強運の博徒(人間): 3つの出目が出ていたら1つ選ぶ（盤面を見ながら選ぶUIを駆動）
    if (run('!!state.gamblerChoices')) run(`chooseGamblerDie(${seed % 3})`);
    if (!run('state.rolled')) throw new Error(`${tag} ダイスを振っても確定しない（gambler選択の滞留）`);
    drain(tag);
